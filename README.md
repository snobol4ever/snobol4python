...
snobol4python is the Python library arm of the [snobol4ever](https://github.com/snobol4ever) project. The full SNOBOL4/SPITBOL *language* — complete compiler, runtime, GOTO execution model, DEFINE/DATA/FIELD, CODE(), EVAL(), TRACE — is available on the JVM via [snobol4jvm](https://github.com/snobol4ever/snobol4jvm) and on .NET via [snobol4dotnet](https://github.com/snobol4ever/snobol4dotnet). The native compiler targeting x86-64 ASM, JVM bytecode, and .NET MSIL simultaneously is [snobol4x](https://github.com/snobol4ever/snobol4x).

The pattern engine underlying snobol4python implements the **Byrd Box model** — four execution states per pattern node: **α** (proceed), **β** (recede), **γ** (succeed), **ω** (concede) — first described by Lawrence Byrd in 1980 and generalized by Todd Proebsting in 1996 as a code generation strategy for goal-directed languages. The C/SPIPAT backend runs this model in C; the pure-Python backend runs it as a Python state machine. Same semantics, different substrates.
